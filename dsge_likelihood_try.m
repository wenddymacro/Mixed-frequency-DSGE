...
%------------------------------------------------------------------------------
ipred = Model.nstatic+(1:Model.nspred)';
[A,B] = kalman_transition_matrix(DynareResults.dr,ipred,1:Model.nspred,Model.exo_nbr);
%get observable position in decision rules
obs_var=DynareResults.dr.inv_order_var(DynareOptions.varobs_id);
%get observation equation matrices
[C,D] = kalman_transition_matrix(DynareResults.dr,obs_var,1:Model.nspred,Model.exo_nbr);

Pstar_new = lyapunov_solver(A,B,Q,DynareOptions);

%A = A(1:Model.nspred, 1:Model.nspred);
%B = B(1:Model.nspred, 1:Model.exo_nbr);
%C = C(1:length(obs_var), 1:Model.nspred);
%D = D(1:length(obs_var), 1:Model.exo_nbr);
mm = length(A);   
a = zeros(mm,1);
    ...
    if no_missing_data_flag
        ...
        elseif DynareOptions.fast_kalman_filter
            ...
            [LIK,lik] = kalman_filter(Y,diffuse_periods+1,size(Y,2), ...
                                      ...
                                      DynareOptions.presample, ...
                                      A,Q,B,H,Z,mm,pp,rr,Zflag,diffuse_periods, ...
                                      analytic_deriv_info{:});
        end
    else
